//
//  XZZConstants.m
//  Matchedup
//
//  Created by Zac on 12/01/2015.
//  Copyright (c) 2015 1st1k. All rights reserved.
//

#import "XZZConstants.h"

@implementation XZZConstants

#pragma mark - User Profile

NSString *const kXZZUserTagLineKey = @"tagLine";

NSString *const kXZZUserProfileKey = @"profile";
NSString *const kXZZUserProfileNameKey = @"name";
NSString *const kXZZUserProfileFirstNameKey = @"firstName";
NSString *const kXZZUserProfileLocationKey = @"location";
NSString *const kXZZUserProfileGenderKey = @"gender";
NSString *const kXZZUserProfileBirthdayKey = @"birthday";
NSString *const kXZZUserProfileInterestedInKey = @"interestedIn";
NSString *const kXZZUserProfilePictureURLKey = @"pictureURL";
NSString *const kXZZUserProfileRelationsihpStatusKey = @"relationshipStatus";
NSString *const kXZZUserProfileAgeKey = @"age";


#pragma mark - Photo Class

NSString *const kXZZPhotoClassKey = @"Photo";
NSString *const kXZZPhotoUserKey = @"user";
NSString *const kXZZPhotoPictureKey = @"image";

#pragma mark - Activity Class

NSString *const KXZZActivityClassKey = @"Activity";
NSString *const KXZZActivityTypeKey = @"type";
NSString *const KXZZActivityFromUserKey = @"fromUser";
NSString *const KXZZActivityToUserKey = @"toUser";
NSString *const KXZZActivityPhotoKey = @"photo";
NSString *const KXZZActivityTypeLikeKey = @"like";
NSString *const KXZZActivityTypeDislikeKey = @"dislike";

#pragma mark - Settings

NSString *const kXZZMenEnabledKey = @"men";
NSString *const kXZZWomenEnabledKey = @"women";
NSString *const kXZZSingleEnabledKey = @"single";
NSString *const kXZZAgeMaxKey = @"ageMax";

#pragma mark - ChatRoom

NSString *const kXZZChatRoomClassRoomKey = @"ChatRoom";
NSString *const kXZZChatRoomUser1Key = @"user1";
NSString *const kXZZChatRoomUser2Key = @"user2";

#pragma mark - Chat

NSString *const kXZZChatClassKey = @"Chat";
NSString *const kXZZChatChatroomKey = @"chatroom";
NSString *const kXZZChatFromUserKey = @"fromUser";
NSString *const kXZZChatToUserKey = @"toUser";
NSString *const kXZZChatTextKey = @"text";

@end